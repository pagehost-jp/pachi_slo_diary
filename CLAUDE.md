...
### ⚠️ iOSホーム画面アイコンの問題と解決策

#### 問題: PWA削除後もスタンドアロンモードで開いてしまう

**症状**:
- PWA設定（manifest.json、apple-mobile-web-app-capable）を削除したのに、ホーム画面アイコンがアプリ風（全画面、アドレスバーなし）で開く
- Googleログインが無限ループする（`signInWithRedirect` → `getRedirectResult` が null を返す）

**原因**:
1. **PWA設定が有効だった時期**にホーム画面に追加したアイコンは、**古い設定がiPhoneに保存されている**
2. Webサイト側でコードを変更しても、インストール済みアイコンの設定は更新されない
3. iOSのスタンドアロンモードでは、OAuth redirectが新しいWebView instanceを作成し、認証状態を引き継げない

#### 解決方法

##### 方法1: アイコンを削除して再追加（推奨）
1. ホーム画面のアイコンを長押し → 削除
2. Safari設定 → 履歴とWebサイトデータを消去
3. Safariでページを開く → 共有ボタン → ホーム画面に追加

##### 方法2: iPhoneの「ショートカット」アプリを使う（最も確実）⭐
1. 「ショートカット」アプリを開く
2. 新規ショートカット作成
3. アクション追加 → 「URLを開く」
4. アプリのURL（https://○○.github.io/pachi_slo_diary/）を入力
5. ショートカット名を「Maxima」など好きな名前に設定
6. 「ホーム画面に追加」で配置
